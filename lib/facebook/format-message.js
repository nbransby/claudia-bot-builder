...
class Generic extends FacebookTemplate {
  ...
  addButtonByType(title, value, type, options) {
    ...
    if (type === 'web_url') {
      button.url = value;
      button.webview_height_ratio = options.webview_height_ratio;
      button.messenger_extensions = options.messenger_extensions;
      button.fallback_url = options.fallback_url;
      button.webview_share_button = options.webview_share_button;
  ...
  addButton(title, value, options) {
    ...
    if (isUrl(value)) {
      return this.addButtonByType(title, value, 'web_url', options);
    } else {
...
class List extends FacebookTemplate {
  ...
  addDefaultAction(url, additionalProperties) {
    const bubble = this.getLastBubble();
    ...
    bubble.default_action = Object.assign({
      type: 'web_url',
      url: url
  ...
  addButton(title, value, type, additionalProperties) {
    const bubble = this.getLastBubble();
  ...
  addListButton(title, value, type, additionalProperties) {
